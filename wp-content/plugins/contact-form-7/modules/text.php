	...
	foreach ( $options as $option ) {
		if ( preg_match( '%^id:([-0-9a-zA-Z_]+)$%', $option, $matches ) ) {
			...
			$tabindex_att = (int) $matches[1];
                ...
                //@todo:title属性の追加（応急処置レベル）
		} elseif ( preg_match( '%^title:([-0-9a-zA-Z_@\.]+)$%', $option, $matches ) ) {
                        if($matches[1] === 'name'){
                            $title_att .= ' お名前を入力して下さい（必須）';
                        }elseif($matches[1] === 'address'){
                            $title_att .= ' メールアドレスを入力して下さい（必須）';
                        }elseif($matches[1] === 'subj'){
                            $title_att .= ' 件名を入力して下さい';                              
                        }else{
                            $title_att .= ' ' . $matches[1];
                        }
                }
