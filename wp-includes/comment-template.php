 ...
 * @return void
 * 
 * 13/01/02: modified by fumiyasac(Just One Factory)
 * 
 */
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields =  array(
                'author' => '<p><input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" class="autoClear" title="お名前を入力してください（必須）"' . $aria_req . ' /><label>お名前<span class="commentRed">*</span></label></p>',
		'email'  => '<p><input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" class="autoClear" title="メールアドレスを入力してください（必須）"' . $aria_req . ' /><label>E-Mail<span class="commentRed">*</span></label></p>',
		'url'    => '<p><input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" class="autoClear" title="サイトURLを入力してください" /><label>URL</label></p>',
	);

	$required_text = sprintf( ' ' . __('Required fields are marked %s'), '<span class="commentRed">*</span>' );
	$defaults = array(
		'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
                'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'Comment', 'noun' ) . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
		'must_log_in'          => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="remarkText">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="remarkText">' . __( 'Your email address will not be published.' ) . ( $req ? $required_text : '' ) . '</p>',
...
<input name="submit" type="submit" id="submit" value="<?php echo esc_attr( $args['label_submit'] ); ?>" />
